...
class Application:
    ...
    CenterIR = 3     #Input to detect using IR sensor
    endIR = 5        #Input to detect using IR sensor
    ...
    def bOut0(self):
        print("bOut1")
        #GPIO.digitalWrite(Valve0, GPIO.LOW)
        time.sleep(float(self.blowTime))
        #GPIO.digitalWrite(Valve0, GPIO.HIGH)
    def bOut1(self):
        print("bOut2")
         #GPIO.digitalWrite(Valve0, GPIO.LOW)
        time.sleep(float(self.blowTime))
    ...
    def video_loop(self):
     #   print(str(datetime.datetime.now()))
        ...
        if ok:  # frame captured without any errors
            ...
            if os.path.isfile(str(self.Type0)+'autotrained.xml'):             #test for cascade classifier

                if str(self.loopCapture['text']).find('Loop Capture') != -1:
                    #identify image or snapshot for training
                    gray = cv2.cvtColor(frame,cv2.COLOR_BGR2GRAY)
                    type0 = cv2.CascadeClassifier('Type 0autotrained.xml')

                    nuts=type0.detectMultiScale(gray)
                    for (x,y,w,h) in nuts:
                       print('nut detected on'+str(x)+', '+str(y))
                       font = cv2.FONT_HERSHEY_COMPLEX
                       cv2.putText(frame,str(self.Type0),(x+w,y+h),font,1,(250,250,250),2,cv2.LINE_AA)
                ...
                cv2.putText(frame,self.Type0+' not trained',(1,100),cv2.FONT_HERSHEY_COMPLEX,1,(250,250,250),2,cv2.LINE_AA)
